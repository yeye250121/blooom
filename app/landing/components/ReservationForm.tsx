  ...
  useEffect(() => {
    const fetchBlockedDates = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/reservation/blocked-dates`);
        if (res.ok) {
      ...
      if (flowType === 'installation' && !skipDocuments) {
        ...
        for (const fileType of fileTypes) {
          ...
          if (file) {
            ...
            uploadPromises.push(
              fetch(`${API_BASE_URL}/api/reservation/upload`, {
                method: 'POST',
                body: formDataUpload,
              }).then(async (res) => {
                if (res.ok) {
  ...
  const renderCalendar = () => {
    ...
    for (let i = 0; i < dayNames.length; i++) {
      days.push(
        <div
          key={`header-${i}`}
          className={`flex items-center justify-center h-10 text-caption font-medium ${
            i === 0 ? 'text-error' : i === 6 ? 'text-action-primary' : 'text-text-secondary'
          }`}
        >
          {dayNames[i]}
        </div>
    ...
    for (let day = 1; day <= lastDay.getDate(); day++) {
      ...
      days.push(
        <button
          ...
          className={`
            h-12 rounded-xl text-body font-medium transition-all
            ${isBlocked
              ? 'text-text-tertiary cursor-not-allowed'
              : isSelected
                ? 'bg-action-primary text-white'
                : dayOfWeek === 0
                  ? 'text-error hover:bg-white/80'
                  : dayOfWeek === 6
                    ? 'text-action-primary hover:bg-white/80'
          ...
          {flowType === 'installation' && formData.reservationDate && (
            <div className="w-full bg-white rounded-2xl p-5 space-y-4 shadow-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-body text-text-primary">{formData.address}</span>
              </div>
            </div>
          )}
        </div>
            ...
            <div className="bg-white rounded-2xl p-4 mb-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-bg-primary transition-colors"
                >
              ...
              <div className="fixed bottom-0 left-0 right-0 p-5 bg-gradient-to-t from-bg-primary via-bg-primary">
                <div className="max-w-md mx-auto">
                  <button
                    type="button"
                    onClick={goNext}
                    className="w-full h-14 bg-action-primary hover:bg-action-primary-hover text-white text-title rounded-2xl transition-colors shadow-lg"
                  >
                    {formData.reservationDate.toLocaleDateString('ko-KR', {
                      month: 'long',
                      day: 'numeric',
        ...
        {currentStepName === 'address' && (
          <div className="animate-scale-in">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm">
                ...
                {formData.address ? (
                  <button
                    ...
                    className="w-full h-14 px-4 bg-bg-primary rounded-xl text-left flex items-center justify-between"
                  >
                    <span className="text-body text-text-primary truncate">{formData.address}</span>
                    <span className="text-caption text-action-primary flex-shrink-0 ml-2">변경</span>
                  </button>
                ) : (
                  <button
                    type="button"
                    onClick={openAddressSearch}
                    className="w-full h-14 px-4 bg-bg-primary rounded-xl text-body text-text-tertiary text-left flex items-center gap-2 hover:bg-bg-primary/70 transition-colors"
                  >
              ...
              {formData.address && (
                <div className="bg-white rounded-2xl p-5 shadow-sm">
                  <label className="text-caption text-text-secondary block mb-2">상세 주소</label>
                  <input
                    type="text"
                    ...
                    className="w-full h-14 px-4 bg-bg-primary rounded-xl text-body text-text-primary placeholder:text-text-tertiary focus:outline-none focus:ring-2 focus:ring-action-primary/30"
                  />
                </div>
              )}
            </div>
          ...
          <div className="animate-scale-in">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-action-primary/10 flex items-center justify-center">
                      <IdCardIcon className="w-5 h-5 text-action-primary" />
                    </div>
                    <div>
                      <h4 className="text-title text-text-primary">신분증</h4>
                      <p className="text-caption text-text-secondary">주민등록증이나 운전면허증</p>
                    </div>
                  </div>
                  <label
                    className={`px-4 py-2 rounded-xl text-caption font-medium cursor-pointer transition-colors ${
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-action-primary/10 flex items-center justify-center">
                      <CreditCardIcon className="w-5 h-5 text-action-primary" />
                    </div>
                    <div>
                      <h4 className="text-title text-text-primary">결제 수단</h4>
                      <p className="text-caption text-text-secondary">결제할 카드나 통장 사본</p>
                    </div>
                  </div>
                  <label
                    className={`px-4 py-2 rounded-xl text-caption font-medium cursor-pointer transition-colors ${
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-text-tertiary/10 flex items-center justify-center">
                      <BuildingIcon className="w-5 h-5 text-text-secondary" />
                    </div>
                    <div>
                      <h4 className="text-title text-text-primary">사업자등록증</h4>
                      <p className="text-caption text-text-secondary">사업자 계약 시에만 첨부해주세요</p>
                    </div>
                  </div>
                  <label
                    className={`px-4 py-2 rounded-xl text-caption font-medium cursor-pointer transition-colors ${
                  ...
                  >
                    {formData.documents.businessLicense ? (
                      <span className="flex items-center gap-1">
                        <CheckIcon className="w-4 h-4 text-status-done" /> 첨부됨
                      </span>
                    ) : '첨부하기'}
                    <input
                      type="file"
            ...
            <div className="mt-8 space-y-3">
              ...
              <button
                ...
                className="w-full h-14 text-text-secondary text-body hover:text-text-primary transition-colors"
              >
                나중에 제출할게요
              </button>
            </div>
